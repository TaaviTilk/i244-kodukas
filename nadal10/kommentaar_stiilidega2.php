...
<body>
    ...
    <form method="POST" action="?">
        ...
        <br/>
        <input type="color" name="bg" id="bg" value="<?php echo htmlspecialchars($text_bg);?>"> 
        <label for="bg">Taustavärvus</label>
        <br/>
        <input type="color" name="tc" id="tc" value="<?php echo htmlspecialchars($text_color);?>"> 
        <label for="tc">Tekstivärvus</label>
        <br/>
        <fieldset>
            <legend>Piirjoon</legend>
            <input type="number"  min="0" max="20" step="1" name="bw" id="bw" value="<?php echo htmlspecialchars($border_width);?>">
            <label>Piirjoone laius (0-20px)</label>
            <br/>
            <select name="bs">
                <?php foreach($stiilid as $stiil):?>
                    
                    <option <?php if($border_style == $stiil) echo "selected"; ?>><?php echo $stiil; ?></option>
                    
                <?php endforeach; ?>
            </select>
            <br/>
            <input type="color" name="bc" id="bc" value="<?php echo htmlspecialchars($border_color);?>"> 
            <label for="bc">Piirjoone värvus</label>
            <br/>
            <input type="number" min="0" max="100" step="1" name="br" id="br" value="<?php echo htmlspecialchars($border_radius);?>">
